...
/*
InvoicesApiService CreateInvoiceAttachment
 ...
 * @param request
 * @param imageFile
 * @param invoiceId The ID of the [invoice](entity:Invoice) to attach the file to.
@return CreateInvoiceAttachmentResponse
*/
func (a *InvoicesApiService) CreateInvoiceAttachment(ctx context.Context, request CreateInvoiceAttachmentRequest, imageFile *os.File, invoiceId string) (CreateInvoiceAttachmentResponse, *http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Post")
		localVarPostBody   interface{}
		localVarFileName   string
		localVarFileBytes  []byte
		localVarReturnValue CreateInvoiceAttachmentResponse
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/v2/invoices/{invoice_id}/attachments"
	localVarPath = strings.Replace(localVarPath, "{"+"invoice_id"+"}", fmt.Sprintf("%v", invoiceId), -1)
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v CreateInvoiceAttachmentResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"));
				if err != nil {
					newErr.error = err.Error()
					return localVarReturnValue, localVarHttpResponse, newErr
